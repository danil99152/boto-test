...
- `POST /shorten` — принимает длинный URL и возвращает короткий (есть опция задать свой код);
- `GET /{code}` — выполняет HTTP‑редирект по короткому коду;
- `PATCH /{code}` — обновляет исходный URL для существующего кода;
- `DELETE /{code}` — удаляет короткую ссылку.
...
- `code` — необязательный параметр, желаемый короткий код (должен быть уникальным).

**Ответ (JSON):**

```json
{
  "short_url": "http://localhost:8000/MyCustom1"
}
```

Если `code` не передан, код генерируется псевдослучайно (буквы латинского алфавита + цифры, длина 8 символов). Коллизии при генерации обрабатываются повторными попытками; в логи пишется предупреждение.  
Если `code` уже существует, возвращается `409 Conflict` с сообщением `"Code already exists"`.
...
Обновляет исходный URL для существующей короткой ссылки.

**Запрос (JSON):**

```json
{
  "url": "https://example.org/new/path"
}
```

- **Успех**: `200 OK` с обновлённым `short_url`.
- **Ошибка**: `404 Not Found`, если кода не существует.
...
- невалидный URL (422);
- многократное создание ссылок для одного и того же URL;
- создание ссылки с заданным кодом и конфликт по коду (409);
- обновление существующей/несуществующей ссылки;
- удаление существующей/несуществующей ссылки.
...
- `app/repository.py` — бизнес-логика работы с БД: генерация кода, создание/обновление/удаление записи, поиск оригинального URL.
- `app/schemas.py` — pydantic-схемы запросов/ответов (`ShortenRequest` с опциональным кодом, `ShortenResponse`, `UpdateUrlRequest`).
- `app/main.py` — определение FastAPI-приложения, маршрутов, логирования и точки входа.
- `tests/test_shortener.py` — pytest-тесты эндпоинтов, включая бонусные сценарии.
...
- успешное создание коротких ссылок (в том числе с заданным кодом) и коллизии кодов;
- операции редиректа, обновления и удаления;
- попытки доступа к несуществующим кодам.
...
- **Логирование ключевых событий**: создание/обновление/удаление ссылок, коллизии кодов, редиректы и ошибки попадают в логи.
- **Тесты для основных и бонусных сценариев**: покрыт happy-path, ошибки, валидация и управление ссылками.

**Минусы и ограничения**

- **Нет авторизации и управления пользователями**: сервис полностью открытый, любые запросы могут создавать/менять ссылки.
...
**Что не готово/сделано минимально**

- Нет вспомогательного интерфейса/панели администратора.
...
- Добавил(а) бы **учёт пользователей** (простая аутентификация + привязка ссылок к владельцу).
